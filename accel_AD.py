...
def read_data():
    ...
    fname = "img2.hdf5" #pdir + "BXP5401-data.hdf5"
    f = h5py.File(fname, "r")
    imgs = f["front_cam"][:]
    f.close()

    X = np.array([1.0,1.0])#np.fromfile(pdir + "accel_X.dat", np.float64)
    Y = np.array([1.0,1.0])#np.fromfile(pdir + "accel_Y.dat", np.float64)
    Z = np.array([1.0,1.0])#np.fromfile(pdir + "accel_Z.dat", np.float64)

    df = pd.DataFrame({'X': X, 'Y': Y, 'Z': Z, 'imgs': list(imgs)})
    g = 9.81
    df['accel'] = np.sqrt(df.X**2 + df.Y**2 + (df.Z-g)**2)
    df['backward'] = (df.X+df.Y) < 0.0
    threshold = df.accel.mean() + 2 * df.accel.std()
